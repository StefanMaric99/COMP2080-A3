...
public class NodeList implements List {
	...
	/**
	 * Checks if position is valid for this list and converts it to DNode if it is
	 * valid; O(1) time
	 */
	...
	/**
	 * True if and only if there is an element before the current position; O(1)
	 * time
	 */
	public boolean hasPrev(Position p) throws InvalidPositionException, BoundaryViolationException {
		DNode v = checkPosition(p);
		DNode prev = v.getPrev();
		if (prev == header)
			return false;
	...
	public Position next(Position p) throws InvalidPositionException, BoundaryViolationException {
		...
		if (next == trailer)
			throw new BoundaryViolationException("Cannot advance past the back of the list");
		return next;
	}
	 ...
	 * iterator.
	 */
	public boolean hasNext(Position p) throws InvalidPositionException, BoundaryViolationException {
		DNode v = checkPosition(p);
		DNode next = v.getNext();
		if (next == trailer)
			return false;
	...
	/**
	 * Inserts an element after the given node, returning the new position; O(1)
	 * time
	 */
	...
	/**
	 * Inserts and element at the back of the list, returning the new position; O(1)
	 * time
	 */
	...
	/**
	 * Replace the element at the given position with the new element and return the
	 * old element; O(1) time
	 */
